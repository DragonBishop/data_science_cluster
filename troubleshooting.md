...
* **Database connections fail with a hostname mismatch when using `sslmode=verify-full`**
  ...
  * **How to fix it:** Check `dnsNames` and `ipAddresses` in `apps/databases/postgres-tls.yaml`. `postgis-cluster-rw`, `-ro`, and `-r` are covered in both short and fully-qualified forms, along with `localhost`, `postgres.internal`, `127.0.0.1`, and the shared Gateway's LAN IP. Adding a name there causes cert-manager to reissue the certificate. If the cluster was rebuilt, the CA also changed, re-run the `root.crt` command in Step 7.
